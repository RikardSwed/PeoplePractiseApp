import { store } from "./storage.js?v=1.0.0";
import { initFileTest } from "./file-test.js?v=1.0.0";
import { APP_VERSION, initVersionInfo } from "./version.js?v=1.0.0";
...
let activityFilter = "all", activityCategory = "all", activitySort = "newest", activityContentMode = "activities", calendarMode = "month";
...
function renderCalendar() {
  ...
  renderCalendarAgenda(plans,events);
}
...
function initBackupTools(){const fileInput=$("#import-data-file");$("#export-data").onclick=()=>{const backup={format:"social-circle-backup",exportVersion:1,appVersion:APP_VERSION,exportedAt:new Date().toISOString(),database:store.snapshot()},blob=new Blob([JSON.stringify(backup,null,2)],{type:"application/json"}),url=URL.createObjectURL(blob),link=document.createElement("a");link.href=url;link.download=`social-circle-backup-${todayKey}.json`;document.body.append(link);link.click();link.remove();setTimeout(()=>URL.revokeObjectURL(url),1000);setBackupStatus(`Säkerhetskopian social-circle-backup-${todayKey}.json har skapats.`,"success");};$("#import-data").onclick=()=>fileInput.click();fileInput.onchange=async()=>{const file=fileInput.files?.[0];if(!file)return;if(file.size>20*1024*1024){setBackupStatus("Filen är större än 20 MB och importerades inte.","error");fileInput.value="";return;}try{const parsed=JSON.parse(await file.text()),database=parsed?.format==="social-circle-backup"?parsed.database:parsed;if(!confirm("Importen ersätter all nuvarande lokal data. Vill du fortsätta?")){fileInput.value="";return;}store.replace(database);setBackupStatus(`Importerade ${file.name}. All data är nu sparad i localStorage.`,"success");}catch(error){setBackupStatus(`Filen kunde inte importeras: ${error.message}`,"error");}fileInput.value="";};$("#clear-local-data").onclick=()=>{if(!confirm("Vill du tömma all Social Circle-data i den här webbläsaren? Exportera gärna en säkerhetskopia först."))return;store.clear();setBackupStatus("All lokal Social Circle-data har tagits bort.","success");};}
...
$("#place-form").onsubmit=e=>{if(e.submitter?.value==="cancel")return;e.preventDefault();const d=new FormData(e.currentTarget),places=read(keys.places),item={id:d.get("id")||uid(),name:d.get("name").trim(),address:d.get("address").trim(),category:d.get("category"),notes:d.get("notes").trim()},index=places.findIndex(p=>p.id===item.id);index>=0?places[index]=item:places.push(item);write(keys.places,places);$("#place-dialog").close();};
...
const formatted=prettyDate(todayKey);$("#today-date").textContent=formatted.charAt(0).toUpperCase()+formatted.slice(1);store.subscribe(renderAll);window.addEventListener("hashchange",showView);initFileTest();initBackupTools();initVersionInfo();showView();renderAll();
